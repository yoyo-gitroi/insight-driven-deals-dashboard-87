...
// Advanced utility function for calculating confidence scores from signals
export function calculateConfidenceScore(signalsJson: string): number {
  try {
    const parsed = safeJsonParse(signalsJson, {});
    
    // Check if it's an object with a 'signals' array inside
    if (parsed?.signals && Array.isArray(parsed.signals)) {
      if (parsed.signals.length === 0) return 0;
      
      const confidenceValues = parsed.signals
        .map(signal => {
          // Check direct confidence field
          if (signal.confidence) {
            return parseFloat(signal.confidence);
          }
          
          // Check for confidence in objection resolution
          if (signal.objection_analysis?.confidence_in_resolution) {
            return parseFloat(signal.objection_analysis.confidence_in_resolution);
          }
    ...
    console.error("Error calculating confidence score:", e);
    return 0;
  }
}

// Extract action recommendations from actions JSON
export function extractActionRecommendations(actionsJson: string) {
  try {
    const parsed = safeJsonParse(actionsJson, {});
    
    let actions = [];
    
    // Handle different possible structures of the actions data
    ...
    return actions.map(action => ({
      verb: action.action_verb || "Unknown",
      priority: action.priority || "medium",
      impact: action.estimated_impact || "medium",
      successRate: action.historical_success_rate || 0.5
    }));
  } catch (e) {
    console.error("Error extracting action recommendations:", e);
    return [];
  }
}
